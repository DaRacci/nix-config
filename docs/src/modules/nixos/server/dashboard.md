...
The dashboard module integrates with [Dashy](https://dashy.to/) and collects dashboard sections from all servers in the cluster to display on the `ioPrimaryHost`. This allows each server to define its own dashboard items, which are then automatically collected and displayed on a single unified dashboard.
