...
| `proxy-extension-behavior`   | Scenario, two-node       | localhost→`nixcloud:8080` rewrite via admin API config inspection, API-key auth (`/health` bypass, `/api/data` 401/200 with `Req-API-Key`), L4 TCP forwarding roundtrip to `nixcloud:9090` echo server, extension priority ordering in Caddy output per `server.proxy.extensions` priority values            |
| `monitoring-pipeline`        | Scenario, two-node       | Prometheus `/api/v1/targets` active (nixio:9100 node, nixio:2019 caddy, nixio:9187 postgres), log shipping via `logger`→Loki `/loki/api/v1/query_range` content+host label, alert rules loaded via `/api/v1/rules`; OTLP feasibility-gated (config inspection only, runtime ingestion deferred)              |
| `guardian-drain-lifecycle`   | Scenario, two-node       | Postgres stop triggers `io-database-coordinator` ExecStop drain, undrain on start, systemd target gating (`io-databases.target`), application-level PG ping (`psql -c "SELECT 1"`), PSK-configurable deterministic auth for test injection                                                                   |
| `seaweedfs-storage-behavior` | Scenario, single-node    | Master leader election via `weed shell cluster.status`, bucket creation cross-surface visibility (S3 API, filer HTTP, volume), object upload/retrieval roundtrip with content match, S3 bucket listing, Admin UI on :23646, FUSE mount I/O (`/dev/fuse` available in QEMU), volume/master restart resilience |
