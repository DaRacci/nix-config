...
in
{
  programs.firefox = {
    ...
    languagePacks = [ "en-AU" "en-US" ];

    profiles = trivial.pipe (config.users.users) [
      (mapAttrs (name: user: {
        inherit name;
        value = {
          containers = {
          ...
          policies =
            let
              Extensions = (trivial.pipe [
                ...
                (map (arr: { id = elemAt 1 arr; name = elemAt 0 arr; }))
                (map (ext: nameValuePair (ext.name) { inherit (ext) id; }))
                listToAttrs
              ]);
            in
            {
              ...
              ExtensionSettings = mergeAttrs [
                (trivial.pipe [
                  # Privacy / Security
                  [ "ublock-origin" "uBlock0@raymondhill.net" ]
                  [ "istilldontcareaboutcookies" "idcac-pub@guus.ninja" ]
                  [ "clearurls" "{74145f27-f039-47ce-a470-a662b129930a}" ]
                  [ "skipredirect@sblask" "{b02bf394-4c30-41f5-8e86-87ed558427c2}" ]
                  ...
                  (map (ext: mkSimpleExtension ext.id ext.name))
                  listToAttrs
                ])
                {
                  "*" = {
  ...
  user.persistence =
    let
      ...
      onEachProfile = value: map (profile: ".mozilla/firefox/${profile}/${profile}") profiles;
    in
    {
