...
let
  ...
  mkRuleString = name: value: "${name} = ${toHyprValue value}";
  ...
  mkMatcherString =
    ...
    if builtins.length (builtins.attrValues nonNullMatchers) == 0 then
      ...
      lib.concatStringsSep "\n" (
        lib.mapAttrsToList (
          name: value: "match:${lib.mine.strings.toSnakeCase name} = ${toHyprValue value}"
        ) nonNullMatchers
      );
  ...
  toHyprValue =
    ...
    if (builtins.typeOf value == "bool") then
      ...
      "${lib.concatStringsSep " " (lib.mapAttrsToList (_name: toString) value)}"
    else
      toString value;
...
in
{
  ...
  config = {
    wayland.windowManager.hyprland.extraConfig =
      cfg.windowrule
      |> lib.mapAttrsToList (_windowRuleName: mkWindowRuleString)
      |> lib.flatten
      |> concatStringsSep "\n";
  };
}
