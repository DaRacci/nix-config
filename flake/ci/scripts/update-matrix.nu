...
def main [
  ...
  eval_expresion: string,                   # The nix expression path to get the list of entries to use for the matrix (e.g., "nixosConfigurations")
  ...
  let eval_entries = nix eval --json $eval_expresion --apply $eval_apply | from json
  if ($eval_entries | describe -d | get type) != "list" {
    log error $"Evaluated expression ($eval_expresion) with apply ($eval_apply) did not return a list"
    exit 1
  }
  ...
  let updated_yaml = $yaml | update-in ["matrix", $matrix_key] $new_matrix
