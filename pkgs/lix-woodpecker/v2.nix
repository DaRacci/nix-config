...
let
  ...
  users = {
    ...
    root = {
      uid = 0;
      shell = if staticShellLayer then "/bin/sh" else "${pkgs.bashInteractive}/bin/bash";
      home = "/root";
      gid = 0;
      groups = [ "root" ];
  ...
  baseSystem =
    ...
    pkgs.runCommand "base-system"
      ...
      (
        ...
        ''
        + (
          if staticShellLayer then
            ''
              cp ${pkgs.pkgsStatic.bashInteractive}/bin/bash $out/bin/sh
              cp ${pkgs.pkgsStatic.coreutils}/bin/env $out/usr/bin/env
            ''
          else
            ''
  ...
  image = nix2container.buildImage {
    ...
    config = {
      Cmd = [ (if staticShellLayer then "/bin/sh" else "/root/.nix-profile/bin/bash") ];
      Env = [
        "USER=root"
        "PATH=${
          lib.concatStringsSep ":" [
