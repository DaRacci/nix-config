...
- [x] 1.2 Add import line in `modules/nixos/server/proxy/default.nix`: `(importModule ./extensions/api-key-auth.nix { inherit proxyLib; })`

## 2. Vhost Options

- [x] 2.1 Declare `options.server.proxy.virtualHosts.<name>.requireApiKey` as `nullOr (submodule { enable (bool, default false), bypassPaths (listOf str, default []) })` — follows kanidm's vhost option declaration pattern
- [x] 2.2 Default value is `null` (auth disabled unless explicitly configured)

## 3. Extension Self-Registration

- [x] 3.1 Set `server.proxy.extensions.api-key-auth` with:
...
- [x] 4.1 Implement `config` function (`name -> vh -> hostCfg -> string`):
...
- [x] 5.1 Implement `globalConfig` function (`hostCfg -> string`):
...
- [x] 6.1 Generate `sops.secrets` entries for each api-key vhost:
...
- [x] 7.1 Configure `systemd.services.caddy.serviceConfig.LoadCredential` for each api-key secret:
...
- [x] 8.1 Run `nix build .#nixosConfigurations.nixio.config.system.build.toplevel` to verify the nixio host builds successfully
- [x] 8.2 Run `nix fmt .` and verify clean format
- [x] 8.3 Verify that enabling `requireApiKey` on a vhost that also has `kanidm != null` triggers the existing mutual exclusivity assertion
